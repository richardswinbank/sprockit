...
BEGIN
  ...
  BEGIN
    ...
    -- log the property value update
    DECLARE @propName NVARCHAR(128);
    DECLARE @propType NVARCHAR(255);

    SELECT 
      @msg = 'Updated property "' + REPLACE(i.PropertyName, '''', '''''') + '"; changed value from "' 
        + REPLACE(d.PropertyValue, '''', '''''') + '" to "' + REPLACE(i.PropertyValue, '''', '''''') + '"'
  ...
  BEGIN
    SELECT 
      @msg = 'Created property "' + i.PropertyName + '" with value "' + i.PropertyValue + '"'
    , @propName = i.PropertyName
    , @propType = i.DataType
    FROM inserted i;
  END
  ...
  BEGIN
    SELECT 
      @msg = 'Deleted property "' + d.PropertyName + '" with value "' + d.PropertyValue + '"'
